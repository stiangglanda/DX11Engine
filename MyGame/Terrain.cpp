...
void Terrain::Init(ID3D11Device* device, ID3D11DeviceContext* dc, const InitInfo& initInfo)
{
	...
	d3dHelper::CreateTextureViewArrayFromFiles(device, dc, layerFilenames, &mLayerMapArraySRV);
...
void Terrain::Draw(ID3D11DeviceContext* dc, const Camera& cam, DirectionalLight lights[3], ID3D11ShaderResourceView* ssao, ID3D11ShaderResourceView* shadow, CXMMATRIX shadowtrans)
{
	dc->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_4_CONTROL_POINT_PATCHLIST);
	dc->IASetInputLayout(InputLayouts::Terrain);
	...
	auto toTexSpace = XMMatrixScaling(0.5f, -0.5f, 1.0f) * XMMatrixTranslation(0.5f, 0.5f, 0);
	...
	//Effects::TerrainFX->SetshadowTransform(world*shadowtrans);//meight not be right
	Effects::TerrainFX->SetViewProj(viewProj);
	Effects::TerrainFX->SetEyePosW(cam.GetPosition());
	Effects::TerrainFX->SetDirLights(lights);
	Effects::TerrainFX->SetFogColor(Colors::Silver);
	...
	Effects::TerrainFX->SetBlendMap(mBlendMapSRV);
	Effects::TerrainFX->SetHeightMap(mHeightMapSRV);
	//Effects::TerrainFX->SetSsaoMap(ssao);
	//Effects::TerrainFX->SetShadowMap(shadow);

	Effects::TerrainFX->SetMaterial(mMat);

	ID3DX11EffectTechnique* tech = Effects::TerrainFX->Light1ShadowTech;
    D3DX11_TECHNIQUE_DESC techDesc;
    tech->GetDesc( &techDesc );
...
//void Terrain::DrawSSAO(ID3D11DeviceContext* dc, const Camera& cam, DirectionalLight lights[3])
//{
//	dc->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_4_CONTROL_POINT_PATCHLIST);
//	dc->IASetInputLayout(InputLayouts::Terrain);
//
...
//	Effects::TerrainFX->SetMaterial(mMat);
//
//	ID3DX11EffectTechnique* tech = Effects::TerrainFX->NormalDepthTech;
//	D3DX11_TECHNIQUE_DESC techDesc;
//	tech->GetDesc(&techDesc);
//
//	for (UINT i = 0; i < techDesc.Passes; ++i)
...
//}
//
//void Terrain::DrawShadow(ID3D11DeviceContext* dc, const Camera& cam, DirectionalLight lights[3], CXMMATRIX viewProje)
//{
//	dc->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_4_CONTROL_POINT_PATCHLIST);
//	dc->IASetInputLayout(InputLayouts::Terrain);
//
...
//	//Effects::TerrainFX->SetTexTransform(XMMatrixScaling(8.0f, 10.0f, 1.0f));
//
//	Effects::TerrainFX->SetLayerMapArray(mLayerMapArraySRV);
//	Effects::TerrainFX->SetBlendMap(mBlendMapSRV);
//	Effects::TerrainFX->SetHeightMap(mHeightMapSRV);
