...
private:
	...
	static constexpr int SMapSize = 2048;
	ShadowMap* mSmap;
...
SkinnedMeshApp::SkinnedMeshApp(HINSTANCE hInstance)
	: D3DApp(hInstance), mSky(nullptr), mRandomTexSRV(nullptr),
	  mFlareTexSRV(nullptr), mCharacterModel(nullptr), mShapesVB(nullptr), mShapesIB(nullptr), mSkullVB(nullptr),
	  mSkullIB(nullptr), mScreenQuadVB(nullptr), mScreenQuadIB(nullptr),
	  mStoneTexSRV(nullptr), mBrickTexSRV(nullptr),
	  mStoneNormalTexSRV(nullptr), mBrickNormalTexSRV(nullptr),
	  mSmap(nullptr), mSsao(nullptr), mLightRotationAngle(0.0f), mSkullIndexCount(0),
	  mWalkCamMode(false)
{
...
bool SkinnedMeshApp::Init()
{
	...
	mAssimpModel->Load(md3dDevice, md3dImmediateContext, &mTexMgr, "Models\\Wolf\\Wolf_dae.dae",
	                   L"Models\\Wolf\\textures\\");

	mSkelAssimpModel = new SkinnedMesh();
	AssimpMesh ret;
	mSkelAssimpModel->LoadMesh(md3dDevice, md3dImmediateContext, &mTexMgr, "Models\\model.dae",
	                           L"Models\\Wolf Rigged and Game Ready\\textures\\");
...
void SkinnedMeshApp::UpdateScene(float dt)
{
	...
	if (GetAsyncKeyState(VK_SPACE))
	{
		...
		{
			PxRigidDynamic* prd;
			prd = mPhx.get()->createDynamic(
				PxTransform(PxVec3(mCam.GetPosition().x, mCam.GetPosition().y, mCam.GetPosition().z)),
				PxSphereGeometry(1), PxVec3(mCam.GetLook().x, mCam.GetLook().y, mCam.GetLook().z) * 50);
...
void SkinnedMeshApp::DrawScene()
{
	...
	mSpriteBatch.DrawString(md3dImmediateContext, mFont, text, textPos, XMCOLOR(0xffffff00));
...
void SkinnedMeshApp::DrawSceneToShadowMap()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		for (auto entity : rview)
		{
			auto& shap = rview.get<component::Shape>(entity);
			auto& trans = rview.get<component::Transform>(entity);
			if (scn.registry.any_of<component::RigidPhysics>(entity) && scn.registry.get<
				component::RigidPhysics>(entity).enablePhx)
			{
