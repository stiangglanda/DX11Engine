...
void M3DLoader::ReadSkinnedVertices(std::ifstream& fin, UINT numVertices,
                                    std::vector<Vertex::PosNormalTexTanSkinned>& vertices)
{
	std::string ignore;
	vertices.resize(numVertices);
	...
	for (UINT i = 0; i < numVertices; ++i)
	{
		...
		vertices[i].BoneIndices[0] = static_cast<BYTE>(boneIndices[0]);
		vertices[i].BoneIndices[1] = static_cast<BYTE>(boneIndices[1]);
		vertices[i].BoneIndices[2] = static_cast<BYTE>(boneIndices[2]);
		vertices[i].BoneIndices[3] = static_cast<BYTE>(boneIndices[3]);
	}
}
