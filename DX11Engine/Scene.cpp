	...
	if (ImGui::BeginPopup("Add Component"))
	{
		...
		if (!registry.any_of<component::RigidPhysics>(static_cast<entt::entity>(selected)))
		{
			if (ImGui::Selectable(component::components[4]))
			{
				...
				XMVECTOR rot;
				XMVECTOR scal;
				XMMatrixDecompose(&scal, &rot, &tr, XMLoadFloat4x4(&trans.Transform));

				XMFLOAT3 xtr;
				XMStoreFloat3(&xtr, tr);


				prd = phx.get()->createDynamic(PxTransform(PxVec3(xtr.x, xtr.y, xtr.z)), PxSphereGeometry(1),
				                               PxVec3(0, 0, 0));
...
void Scene::Init(ID3D11Device* Md3dDevice, ID3D11DeviceContext* Md3dImmediateContext, TextureMgr* TexMgr,
...
void Scene::EditTransform(float* cameraView, float* cameraProjection, float* matrix, bool editTransformDecomposition)
{
	...
	if (useWindow)
	{
		...
		ImGui::PushStyleColor(ImGuiCol_WindowBg, static_cast<ImVec4>(ImColor(0.35f, 0.3f, 0.3f)));
...
void Scene::ShowGuizmo(const Camera& Cam)
{
	...
	for (int matId = 0; matId < 1; matId++)
	{
		...
		XMFLOAT4X4 mBoxWorldguizmo = s.Transform;
