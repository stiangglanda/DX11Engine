...
void Terrain::Draw(ID3D11DeviceContext* dc, const Camera& cam, DirectionalLight lights[3],
                   ID3D11ShaderResourceView* ssao, ID3D11ShaderResourceView* shadow, CXMMATRIX shadowtrans)
{
	dc->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_4_CONTROL_POINT_PATCHLIST);
	dc->IASetInputLayout(InputLayouts::Terrain);
...
bool Terrain::InBounds(int i, int j)
{
	...
	return
		i >= 0 && i < static_cast<int>(mInfo.HeightmapHeight) &&
		j >= 0 && j < static_cast<int>(mInfo.HeightmapWidth);
}
