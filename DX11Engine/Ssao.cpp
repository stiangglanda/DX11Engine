...
Ssao::Ssao(ID3D11Device* device, ID3D11DeviceContext* dc, int width, int height, float fovy, float farZ)
	: md3dDevice(device), mDC(dc), mScreenQuadVB(nullptr), mScreenQuadIB(nullptr), mRandomVectorSRV(nullptr),
	  mNormalDepthRTV(nullptr), mNormalDepthSRV(nullptr), mAmbientRTV0(nullptr), mAmbientSRV0(nullptr),
	  mAmbientRTV1(nullptr), mAmbientSRV1(nullptr)
...
void Ssao::ComputeSsao(const Camera& camera)
{
	...
	mDC->OMSetRenderTargets(1, renderTargets, nullptr);
...
{
	float aspect = static_cast<float>(mRenderTargetWidth) / static_cast<float>(mRenderTargetHeight);
