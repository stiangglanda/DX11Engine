...
public:
	...
	AssimpMesh(ID3D11Device* dev, const std::vector<Vertex::PosNormalTexTanSkinned>& vertices,
	           const std::vector<UINT>& indices, const std::vector<ID3D11ShaderResourceView*> DiffuseMapSRV,
	           const std::vector<ID3D11ShaderResourceView*> NormalMapSRV, std::vector<Material> Mat) :
		vertices(vertices),
		indices(indices),
