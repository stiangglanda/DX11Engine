...
HRESULT SpriteBatch::Initialize(ID3D11Device* device)
{
	...
	hr = device->CreateBuffer(&vbd, nullptr, &mVB);
	if (FAILED(hr))
		return hr;
...
void SpriteBatch::BeginBatch(ID3D11ShaderResourceView* texSRV)
{
	...
	auto tex = reinterpret_cast<ID3D11Texture2D*>(resource);
...
void SpriteBatch::Draw(const POINT& position, PackedVector::XMCOLOR color)
{
	Sprite sprite;
...
void SpriteBatch::Draw(const POINT& position, const CD3D11_RECT& sourceRect, PackedVector::XMCOLOR color)
{
...
void SpriteBatch::Draw(const CD3D11_RECT& destinationRect, PackedVector::XMCOLOR color)
{
	Sprite sprite;
	sprite.SrcRect = CD3D11_RECT(0, 0, mTexWidth, mTexHeight);
...
void SpriteBatch::Draw(const CD3D11_RECT& destinationRect, const CD3D11_RECT& sourceRect, PackedVector::XMCOLOR color)
{
	Sprite sprite;
	sprite.SrcRect = sourceRect;
	sprite.DestRect = destinationRect;
...
void SpriteBatch::DrawString(ID3D11DeviceContext* dc, FontSheet& fs, const std::wstring& text,
                             const POINT& pos, PackedVector::XMCOLOR color)
{
...
void SpriteBatch::DrawBatch(ID3D11DeviceContext* dc, UINT startSpriteIndex, UINT spriteCount)
{
	...
	auto v = reinterpret_cast<SpriteVertex*>(mappedData.pData);
...
XMFLOAT3 SpriteBatch::PointToNdc(int x, int y, float z)
{
	...
	p.x = 2.0f * static_cast<float>(x) / mScreenWidth - 1.0f;
	p.y = 1.0f - 2.0f * static_cast<float>(y) / mScreenHeight;
	p.z = z;
...
void SpriteBatch::BuildSpriteQuad(const Sprite& sprite, SpriteVertex v[4])
{
	...
	v[0].Tex = XMFLOAT2(static_cast<float>(src.left) / mTexWidth, static_cast<float>(src.bottom) / mTexHeight);
	v[1].Tex = XMFLOAT2(static_cast<float>(src.left) / mTexWidth, static_cast<float>(src.top) / mTexHeight);
	v[2].Tex = XMFLOAT2(static_cast<float>(src.right) / mTexWidth, static_cast<float>(src.top) / mTexHeight);
	v[3].Tex = XMFLOAT2(static_cast<float>(src.right) / mTexWidth, static_cast<float>(src.bottom) / mTexHeight);
