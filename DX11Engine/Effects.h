...
public:
	...
	void SetWorldFrustumPlanes(XMFLOAT4 planes[6])
	{
		WorldFrustumPlanes->SetFloatVectorArray(reinterpret_cast<float*>(planes), 0, 6);
	}
	...
	ID3DX11EffectTechnique* Light3FogShadowTech;
	ID3DX11EffectTechnique* Light1ShadowTech;
	ID3DX11EffectTechnique* Light1FogShadowTech;
...
public:
	...
	~SsaoNormalDepthEffect() override;
...
public:
	...
	void SetOffsetVectors(const XMFLOAT4 v[14])
	{
		OffsetVectors->SetFloatVectorArray(reinterpret_cast<const float*>(v), 0, 14);
	}

	void SetFrustumCorners(const XMFLOAT4 v[4])
	{
		FrustumCorners->SetFloatVectorArray(reinterpret_cast<const float*>(v), 0, 4);
	}
